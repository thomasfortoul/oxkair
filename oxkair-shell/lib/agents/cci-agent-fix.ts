...
export class CCIAgent implements StandardizedAgent {
  ...
  async executeInternal(context: StandardizedAgentContext): Promise<StandardizedAgentResult> {
    ...
    try {
      ...
      return {
        ...
        data: {
          cciResult: cciResult, // Ensure cciResult is properly included in data
          globalFlags: cciResult.globalFlags, // Explicitly include global flags
        },
        errors: [],
        metadata: {
  ...
  private validateGlobalPeriods(
    ...
    logger.logInfo("CCI_AGENT", `Created ${flags.length} global period flags`);
    return flags;
  }
  ...
  private calculateSummary(cciResult: CCIResult): CCISummary {
    ...
    const totalFlags = ptpViolations + mueViolations + globalViolations;
